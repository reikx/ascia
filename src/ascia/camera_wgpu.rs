...
use crate::ascia::core::{AsciaEngine, Camera, CParticle, FlatMaterial, Global, LambertMaterial, LambertWithShadowMaterial, ObjectNode, Polygon, PresetLight, PresetPolygonMaterial, RenderChar, ObjectNodeAttribute, PresetCamera, AsciaEnvironment, PresetObjectNodeAttributeDispatcher, CameraDispatcher, ObjectNodeAttributeDispatcher, PresetCParticleMaterial};
...
impl GPUMemoryConvertStatic<20> for PresetPolygonMaterial {
    #[inline]
    fn convert(&self) -> [u8; 20] {
        let mut buf: [u8; 20] = [0; 20];
        match self {
...
impl GPUMemoryConvertStatic<12> for PresetCParticleMaterial {
    #[inline]
    fn convert(&self) -> [u8; 12] {
        let mut buf: [u8; 12] = [0; 12];
        match self {
            PresetCParticleMaterial::FlatMaterial(m) => {
                buf[0..12].copy_from_slice(&m.color.convert());
            }
...
impl<E: AsciaEnvironment<PolygonMaterials=PresetPolygonMaterial>> GPUMemoryConvertStatic<80> for Polygon<E, Global> {
    #[inline]
...
impl<E: AsciaEnvironment<CParticleMaterials=PresetCParticleMaterial>> GPUMemoryConvertStatic<64> for CParticle<E, Global> {
    #[inline]
    fn convert(&self) -> [u8; 64] {
        let mut buf: [u8; 64] = [0; 64];
        buf[0..12].copy_from_slice(&self.position.convert());
        buf[16..28].copy_from_slice(&self.velocity.convert());
        buf[32..44].copy_from_slice(&self.material.convert());
...
pub struct GPUWrapper<E: AsciaEnvironment, C: Camera<E>> {
    pub cpu_camera: C,
    daq: (Device, Queue),
    shader: ShaderModule,
    _ph: PhantomData<E>,
...
impl<E: AsciaEnvironment<PolygonMaterials=PresetPolygonMaterial, CParticleMaterials=PresetCParticleMaterial, Lights=PresetLight>> GPUWrapper<E, SimpleCamera<E>> {
    pub fn generate(value: SimpleCamera<E>) -> Option<Self> {
        ...
        if let Some(adapter) = pollster::block_on(instance.request_adapter(&wgpu::RequestAdapterOptions::default())) {
            if let Ok(daq) = pollster::block_on(adapter.request_device(&wgpu::DeviceDescriptor {
                ...
                return Some(GPUWrapper {
                    cpu_camera: value,
                    shader: daq.0.create_shader_module(ShaderModuleDescriptor {
                        label: None,
...
impl<E: AsciaEnvironment<PolygonMaterials=PresetPolygonMaterial, CParticleMaterials=PresetCParticleMaterial, Lights=PresetLight>> ObjectNodeAttribute<E> for GPUWrapper<E, SimpleCamera<E>> {}

impl<E: AsciaEnvironment<ObjectNodeAttributes=PresetObjectNodeAttributeDispatcher<E>, PolygonMaterials=PresetPolygonMaterial, CParticleMaterials=PresetCParticleMaterial, Lights=PresetLight, Cameras=PresetCamera<E>>> From<GPUWrapper<E, SimpleCamera<E>>> for PresetObjectNodeAttributeDispatcher<E> {
    fn from(value: GPUWrapper<E, SimpleCamera<E>>) -> Self {
        PresetCamera::SimpleCameraGPU(value).make_attribute_enum()
    }
}

impl<E: AsciaEnvironment<PolygonMaterials=PresetPolygonMaterial, CParticleMaterials=PresetCParticleMaterial, Lights=PresetLight>> Camera<E> for GPUWrapper<E, SimpleCamera<E>> {
    fn render(&self, node: &ObjectNode<E, Global>, engine: &AsciaEngine<E>) -> Vec<Vec<RenderChar>> {
        let device = &self.daq.0;
        ...
        for iter in engine.genesis_global.iter() {
            ...
            if let Some(s) = &*attr_rr {
                if let Some(PresetLight::PointLight(p)) = &s.light() {
        ...
        if c_particles.is_empty() {
            c_particles.push(CParticle {
                ...
                threshold: 0.0,
                mode: SPHERE,
                material: E::CParticleMaterials::default(),
                _ph: Default::default(),
            });
        }
...

        {
            ...
            {
                let mut cpass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor { label: Some("RENDER"), timestamp_writes: None });
        ...
        self.daq.1.submit(Some(encoder.finish()));
...
impl<E: AsciaEnvironment<PolygonMaterials=PresetPolygonMaterial, CParticleMaterials=PresetCParticleMaterial, Lights=PresetLight>> GPUWrapper<E, SimpleBVHCamera<E>> {
    pub fn generate(value: SimpleBVHCamera<E>) -> Option<Self> {
        let instance = wgpu::Instance::new(wgpu::InstanceDescriptor::default());
        let mut limits = wgpu::Limits::default();
        limits.max_bind_groups = 8;
...
impl<E: AsciaEnvironment<PolygonMaterials=PresetPolygonMaterial, CParticleMaterials=PresetCParticleMaterial, Lights=PresetLight>> ObjectNodeAttribute<E> for GPUWrapper<E, SimpleBVHCamera<E>> {}

impl<E: AsciaEnvironment<ObjectNodeAttributes=PresetObjectNodeAttributeDispatcher<E>, PolygonMaterials=PresetPolygonMaterial, CParticleMaterials=PresetCParticleMaterial, Lights=PresetLight, Cameras=PresetCamera<E>>> From<GPUWrapper<E, SimpleBVHCamera<E>>> for PresetObjectNodeAttributeDispatcher<E> {
    fn from(value: GPUWrapper<E, SimpleBVHCamera<E>>) -> Self {
        PresetCamera::SimpleBVHCameraGPU(value).make_attribute_enum()
    }
}

impl<E: AsciaEnvironment<PolygonMaterials=PresetPolygonMaterial, CParticleMaterials=PresetCParticleMaterial, Lights=PresetLight>> Camera<E> for GPUWrapper<E, SimpleBVHCamera<E>> {
    fn render(&self, node: &ObjectNode<E, Global>, engine: &AsciaEngine<E>) -> Vec<Vec<RenderChar>> {
        let device = &self.daq.0;
        ...
        if pointlights.is_empty() {
            ...
            // build_bvh_polygons
            {
                ...
                while dispatch_num >= 1 {
                    let mut cpass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor { label: Some("BVH"), timestamp_writes: None });
